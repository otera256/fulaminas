...
pub trait Optimizer<B: Backend> {
    fn step(&mut self, loss: &Tensor<B>);
}
...
impl<B: Backend + 'static> Optimizer<B> for SGD {
    fn step(&mut self, loss: &Tensor<B>) {
        ...
        let params: Vec<Tensor<B>> = with_graph::<B, _, _>(|graph| {
            graph
                .nodes
                .iter()
        ...
        for param in params {
            ...
            // lr is scalar const.
            let lr_tensor = Tensor::new_const(B::from_vec(vec![self.lr], &[1]));
...
pub struct Adam<B: Backend + 'static> {
    ...
    state: HashMap<NodeId, (Tensor<B>, Tensor<B>)>,
    t: Tensor<B>, // timestep
}
...
impl<B: Backend + 'static> Adam<B> {
    pub fn new(lr: f32) -> Self {
        Self {
            ...
            epsilon: 1e-8,
            state: HashMap::new(),
            t: Tensor::new_parameter(B::from_vec(vec![0.0], &[1])),
        }
    }
}
...
impl<B: Backend + 'static> Optimizer<B> for Adam<B> {
    fn step(&mut self, loss: &Tensor<B>) {
        ...
        // Update timestep
        // t = t + 1
        let one = Tensor::new_const(B::from_vec(vec![1.0], &[1]));
        let new_t = self.t.clone() + one.clone();
        let _ = Tensor::assign(&self.t, &new_t, 0);

        // We use the new t for calculation (t=1, 2, ...)
        // Note: graph is static, but loop execution will update 't'.

        // Constants (Scalars) initialization should happen only once if possible?
        // But step is called once during build. So it's fine.
        let beta1 = Tensor::new_const(B::from_vec(vec![self.beta1], &[1]));
        let beta2 = Tensor::new_const(B::from_vec(vec![self.beta2], &[1]));
        let epsilon = Tensor::new_const(B::from_vec(vec![self.epsilon], &[1]));
        let lr = Tensor::new_const(B::from_vec(vec![self.lr], &[1]));
        ...
        for param in params {
            ...
            if !self.state.contains_key(&id) {
                ...
                let m = Tensor::new_parameter(B::zeros(&shape));
                let v = Tensor::new_parameter(B::zeros(&shape));
                self.state.insert(id, (m, v));
            }

            let (m, v) = self.state.get(&id).unwrap().clone();

            // Update m
            // m = beta1 * m + (1 - beta1) * grad
            let new_m = beta1.clone() * m.clone() + one_minus_beta1.clone() * grad.clone();
            let _ = Tensor::assign(&m, &new_m, 0);

            // Update v
            // v = beta2 * v + (1 - beta2) * grad^2
            let grad_sq = grad.clone() * grad;
            let new_v = beta2.clone() * v.clone() + one_minus_beta2.clone() * grad_sq;
            let _ = Tensor::assign(&v, &new_v, 0);

            // Update p
            // p -= lr * m / (sqrt(v) + eps)
            // Note: Skipping bias correction for MVP as discussed
